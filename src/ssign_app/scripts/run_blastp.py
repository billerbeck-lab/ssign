...
def run_local_blastp(query_fasta, db_path, evalue, exclude_taxid, num_threads=None, failure_log_dir=""):
    ...
    cmd = [
        ...
        # small substrate set (~16-100 proteins) against the DB (Swiss-Prot
        # by default, or opt-in NR), so query-parallel scales linearly with
        # thread count whereas DB-chunk-parallel hits diminishing returns
        # past ~4 threads on small query sets (NCBI BLAST+ user guide 4.2.1).
