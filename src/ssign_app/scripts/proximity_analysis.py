...
def main():
    ...
    # locus_to_info is (contig, gene_index) — typed at store time so the
    # proximity loop below doesn't need to re-cast on every read.
    genes_by_contig: dict[str, list[dict[str, str]]] = {}
    locus_to_info: dict[str, tuple[str, int]] = {}
    with open(args.gene_order) as f:
        for row in csv.DictReader(f, delimiter="\t"):
            ...
            locus_to_info[row["locus_tag"]] = (contig, idx)
    ...
    for comp_locus in ss_component_loci:
        ...
        for offset in range(-args.window, args.window + 1):
            ...
            try:
                dlp_ext_prob = float(pred.get("dlp_extracellular_prob", pred.get("extracellular_prob", 0)))
            ...
            is_dse = dse_type not in ("Non-secreted", "T3SS", "") and dse_max_prob >= args.conf_threshold
