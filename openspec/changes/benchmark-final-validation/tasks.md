...
- [ ] 2.3 Generate the rerun batches + submit command. DECISION NEEDED: the benchmark's `run_benchmark_batch.pbs` uses `--use-input-annotations` (locus_tags = RefSeq, needed for recall reconciliation) but has annotation OFF + no `--enrichment-stats`. The rerun must add `--enrichment-stats` and turn annotation ON (to grade T5SS + the annotation benchmark). Update `make_batches.py` to read `rerun_inputs.txt` + clone the PBS with those flags. Then dry-run + confirm `gpu_type=RTX6000`.
- [ ] 2.4 Transfer the 8 new `.gbff` to CX3 (rsync) + the updated batch/PBS files.
