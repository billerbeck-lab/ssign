...
- [ ] 2.2b Stage the 7 cache-not-staged T5SS genomes from `refseq_cache` into `inputs_gb/` `.gbff` (extend script 22) AND collapse the 2 same-assembly merges into one input each (PAO1, C58 union their replicons) so each rerun unit = one whole assembly.
