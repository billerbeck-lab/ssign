...
**Reason**: The spiky per-type null histogram grid is uninformative at single-genome scale (a count of 0-4 over ~5000 shifts piles on the left). Replaced by fold-enrichment / significance bar charts that carry the same signal at both single-genome and group scale.
**Migration**: Read the per-tool and combined fold-enrichment bar charts (`*_enrichment_fold[_combined].png`) instead of the null-distribution grid.
