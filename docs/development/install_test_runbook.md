...
- **A4. tier 2 (extended), HPC (container path, recommended).** The self-contained image is the fresh-install path now; there are no per-tool native installs. Follow `containers/README.md` § Quickstart (extended): get the `.sif`, `fetch-databases --tier extended` (run from the image), `ssign-setup-dtu --signalp-only`, then `ssign-run ... --tier extended --db-root ... --max-ram <job RAM>` (on PBS use `scripts/cx3/run_container_extended.pbs`, which pins the RAM budget and stages the image). Host-provided pieces: only SignalP 6 + the fetched DBs; Bakta/EggNOG/DeepLocPro/DeepSecE/pLM-BLAST and all their weights are baked. Functional run: one substrate-yielding genome on a GPU node, confirm **23/23** steps (retrieve per cx3-retrieve). Confirm, then wipe. (The native per-tool install via `install.md` stays documented for non-container users, but is no longer the tested HPC path.)
