...
T5SS handling is special: the autotransporter (T5aSS) and chaperone-usher (T5cSS) classes export their own passenger domain
...
Six tools run independently against the substrate set. Each is skip-able with the
`--skip-*` flag.

| Tool | What it adds |
|---|---|
| **BLASTp** | Best hit description against Swiss-Prot. |
...
- **`<sample-id>_results_raw.csv`**: raw outputs from tools run.
...
families. It runs an all-vs-all BLASTp on that
genome's filtered substrate set, keeps hits above the
...
a two-panel conservation figure.
